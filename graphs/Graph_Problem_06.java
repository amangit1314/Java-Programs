...
public class Graph_Problem_06{
    ...
    static boolean isSafe(int row, int col, int[][] maze, int lengthOfMaze, boolean[][] visited) {
        return ( row != -1 && row != lengthOfMaze && col != -1 && col != lengthOfMaze && !visited[row][col] && maze[row][col] != 0) ;
    }

    // Function to print all the possible paths from (0, 0) to (n-1, n-1).
    static void printPathUtil(int row, int col, int[][] maze, int lengthOfMaze, boolean[][] visited) {
        // This will check the initial point (i.e. (0, 0)) to start the paths.
        if (row == -1 || row == lengthOfMaze || col == -1 || col == lengthOfMaze || visited[row][col] || maze[row][col] == 0)
            return;

        // If reach the last cell (n-1, n-1) then store the path and return
        if (row == lengthOfMaze - 1 && col == lengthOfMaze - 1) {
        ...
        if (isSafe(row + 1, col, maze, lengthOfMaze, visited)) {
            path += 'D';
            printPathUtil(row + 1, col, maze, lengthOfMaze, visited);
            path = path.substring(0, path.length() - 1);
        }

        // Check if the left move is valid
        if (isSafe(row, col - 1, maze, lengthOfMaze, visited)) {
            path += 'L';
            printPathUtil(row, col - 1, maze, lengthOfMaze, visited);
            path = path.substring(0, path.length() - 1);
        }
    ...
    static void printPath(int[][] maze, int lengthOfMaze) {
        boolean [][]visited = new boolean[lengthOfMaze][MAX];

        // Call the utility function to find the valid paths
        printPathUtil(0, 0, maze, lengthOfMaze, visited);
