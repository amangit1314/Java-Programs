...
// Program Title => Searching in an array where adjacent differ at most k.
/* example : 
...
public class SSP_Problem_09 {
    ...
    static int search(int[] a, int n, int x, int k) {
        // Traverse the given array starting leftmost element
        int i = 0;
        while (i < n) {
            if (a[i] == x)
                return i;

            // Jump the difference between current array element 
